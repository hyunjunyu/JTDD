...
- 처음에는 Lagrangian L 자체를 경로의 functional로 혼동했으나, L=L(x, x', t)는 function이고 S[x(t)]가 functional임을 교정함.
- Euler-Lagrange 유도에서 x→x+εη일 때 x'→x'+εη'가 되는 이유를 처음에는 설명하지 못했으나 미분으로 확인함.
- least action을 반드시 minimum으로 표현하는 경향이 있었고, 실제 조건은 stationary/extremum이라는 점을 보완함.
- Euler-Lagrange 유도의 마지막 단계인 '모든 허용 η에 대해 적분이 0이면 integrand가 0'이라는 논리를 직관적으로 설명했지만 엄밀한 수학적 조건은 아직 다루지 않음.

## 다음 복습 질문

- Newtonian mechanics가 local, Lagrangian mechanics가 global하다는 말의 정확한 의미를 설명해봐.
- L과 S의 입력이 각각 무엇이며 왜 L은 function, S는 functional인가?
- 왜 variation η(t)는 η(t_i)=η(t_f)=0이어야 하는가?
- x→x+εη이면 x'는 어떻게 변하는가?
- 왜 ∫ f(t)η(t)dt=0이 모든 허용 가능한 η(t)에 대해 성립하면 f(t)=0이어야 하는가?
- Euler-Lagrange equation에 L=T-V를 넣으면 Newton의 제2법칙이 어떻게 복원되는가?
