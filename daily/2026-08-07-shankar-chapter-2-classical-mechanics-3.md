...
updated: 2026-08-07
tags: [learning]
source: "학습 세션 → Issue #12 (수집기: ingest_learning_note.py)"
status: active
kind: mixed
runner: GPT-5.6 Sol
source_issue: 12
...
- L과 S의 역할을 처음에는 혼동했으나, L은 x, x', t의 function이고 S는 경로 x(t)의 functional이라는 구분으로 교정함.
- stationary action을 minimum이라고 표현하는 경향이 있어, extremum/stationary와 minimum의 차이를 계속 확인할 필요가 있음.
- Euler-Lagrange equation에 L=T-V를 대입할 때 ∂L/∂x의 부호를 한 번 놓쳤음.
- canonical momentum을 실제 generalized coordinate에서 계산하고 cyclic coordinate 및 보존법칙과 연결하는 적용 확인이 필요함.

## 다음 복습 질문

- 극좌표에서 q=θ를 잡았을 때 p_θ=∂L/∂θdot가 왜 각운동량과 일치하는가?
- L이 어떤 generalized coordinate q에 직접 의존하지 않을 때 p_q가 왜 보존되는가?
- stationary action은 왜 반드시 minimum을 뜻하지 않는가?
