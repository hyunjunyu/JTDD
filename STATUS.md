...
1. Lagrangian L은 function이고 action S[x(t)]가 functional이라는 구분은 교정 후 이해함.
2. Euler-Lagrange equation의 좌표 일반성은 직교 좌표계에만 제한되지 않고 독립적인 generalized coordinates에 대해 성립한다는 점을 더 익힐 필요가 있음.
3. principle of least action에서 실제로 필요한 조건은 action의 minimum이 아니라 stationary/extremum 조건이라는 점을 더 정확히 구분할 필요가 있음.
4. S의 입력을 단순한 x가 아니라 전체 경로 함수 x(t)라고 엄밀히 표현하기
5. Euler-Lagrange 방정식의 좌표 일반성은 직교 좌표계에만 제한되지 않고 일반화 좌표계까지 포함한다는 점

## 다음 복습 질문 (top 5 — 세션 시작 시 1~2개 예측용)
> 다음 세션에서 다시 물을 것. 러너가 채워 나간다.
1. Newtonian mechanics가 local하고 Lagrangian mechanics가 global하다는 말은 정확히 무슨 뜻인가?
2. 왜 L은 function이고 S는 functional인가?
3. 왜 경로 변분 η(t)는 η(t_i)=η(t_f)=0을 만족해야 하는가?
4. least action에서 왜 minimum보다 stationary action이라고 말하는 편이 더 정확한가?
